...
const url = 'https://192.168.62.13:8443/'
let openRequest = indexedDB.open('db', 1);

openRequest.onupgradeneeded = function () {
    dc = openRequest.result;
    if (!dc.objectStoreNames.contains('history')) {
        dc.createObjectStore('history')
    }
...
openRequest.onsuccess = function (e) {
    ...
    for (let i = 0; i < historya.length; i++) {
        ...
        newelement.innerHTML = historya[i].geocode.name;
        newelement.classList.add('link');
        newelement.addEventListener('click', function () {

            createMarkers3(historya[i].geocode);
            map.flyTo(historya[i].geocode.center, 16);
        });
...
function put(x, key) {
    var request = dc.transaction('history', 'readwrite')
    var store = request.objectStore('history')
    store.put(x, key)
}
...
function get(key) {
    ...
    var request = dc.transaction('history', 'readwrite')
    var store = request.objectStore('history')
    var gety = store.get(key)
    gety.onsuccess = function () {
...
if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(function (position) {
        ...
        routing.options.plan.options.geocoder.options.geocodingQueryParams.lat = lat;
